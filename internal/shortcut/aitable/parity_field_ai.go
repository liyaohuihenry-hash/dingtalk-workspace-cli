...
// aiTasksSubmitted requires one authoritative task ID for every selected field.
// Submitted is an acceptance fact only: existing cell values cannot prove this run completed.
func aiTasksSubmitted(receipt map[string]any, fields []string) bool {
	tasks, ok := findNamedObjectList(receipt, "tasks")
	if !ok || len(tasks) != len(fields) {
		return false
	}
	...
	for _, task := range tasks {
		id := stringValue(task, "fieldId")
		if !wanted[id] || task["status"] != "submitted" || stringValue(task, "taskId") == "" {
			return false
		}
		delete(wanted, id)
	}
