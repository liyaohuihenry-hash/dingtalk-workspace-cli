...
- 本轮从近期起有界扫描 15 个连续 20 天窗口：总项数 1、非空窗口 1、合同异常 0。该安全只读 fixture 关闭了原先“没有非空发件箱”的误判。
...
| `+inbox-list` | fully unlocked | `PASS-RPT-INBOX-PAGES`：16 页、总计 308 后终止；`PASS-RPT-INBOX-ZERO`：0 | public |
| `+outbox-list` | fully unlocked | `PASS-RPT-OUTBOX-HISTORY`：15 个连续窗口、总项 1、非空窗口 1、合同异常 0；`PASS-RPT-OUTBOX-ZERO`：0 | public |
...
| `report +inbox-list` | `dws report +inbox-list --start <start> --end <end> --cursor 0 --size 20 --format json` | `dws report inbox list` 使用完全相同参数；从 `_internalDetailCommands` 取得同页精确 `reportId` | 16 页总计 308，逐页稳定 ID 集合与 next cursor 一致、全局身份无重复；独立未来范围 0/0 且双方明确终止 | `PASS-RPT-INBOX-DOUBLE`；fully unlocked |
...
`+inbox-list` 的 current-HEAD 分页证据为 16 页后明确终止，累计 308，逐页 exact/raw 身份一致且全局身份无重复；`+outbox-list` 的 fixture 扫描为 15 个连续 20 天窗口、总项 1、非空窗口 1、合同异常 0。这里的零项均来自独立保证零范围，不是已知非空集合后的空页。
...
- 归因修正：原先只检查近期窗口，错误地把账号判定为没有安全非空 fixture。本轮从近期起按 20 天窗口有界扫描，在第 15 个窗口发现 1 个非空窗口。
